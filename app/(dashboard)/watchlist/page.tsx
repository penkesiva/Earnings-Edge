...
import { listPendingEarningsCandidates } from '@/lib/earningsDiscovery';
import { AddTickerForm } from './AddTickerForm';
import { BatchImportForm } from './BatchImportForm';
import { EarningsDiscoveryPanel } from './EarningsDiscoveryPanel';
...
export default async function WatchlistPage() {
  ...
  const [{ data: tickers }, candidates] = await Promise.all([
    sb.from('watchlist').select('*').order('ticker', { ascending: true }),
    listPendingEarningsCandidates(sb, user.id),
  ]);
  ...
  return (
    <div className="space-y-5 md:space-y-8">
      ...
      <EarningsDiscoveryPanel candidates={candidates} />

      <div>
        <h2 className="text-sm font-bold tracking-widest uppercase text-fg-subtle mb-3">
          Your watchlist
        </h2>
        <WatchlistMobileList tickers={tickers ?? []} />

        <div className="hidden md:block border border-border mt-2">
          ...
          {tickers?.map(t => (
            <div
              key={t.id}
              className="grid grid-cols-12 gap-4 px-4 py-3 text-sm border-b border-border-subtle items-center"
            >
              ...
              <div className="col-span-3 text-right flex gap-2 justify-end">
                ...
                <form action={deleteTicker}>
                  <input type="hidden" name="id" value={t.id} />
                  <button className="text-xs text-fg-subtle hover:text-signal-sell">DEL</button>
                </form>
              </div>
            </div>
          ))}
          {!tickers?.length && (
            <div className="px-4 py-8 text-center text-fg-subtle text-sm">
              No tickers yet — add one above or pick from upcoming earnings.
            </div>
          )}
        </div>
      </div>

      <details className="group border border-border-subtle bg-bg-elevated">
        <summary className="cursor-pointer px-3 py-2 text-xs tracking-widest text-fg-subtle select-none">
